...
async function renderDashboard() {
  ...
  renderDonut('chartPortStatus', 'legendPortStatus',
    ['Aprovado', 'Em Andamento','Aguardando confirmação', 'Negado'],
    [d.portAprovado ?? 0, d.portAndamento ?? 0, d.portAguardando ?? 0, d.portNegado ?? 0],
    ['#34D399', '#EC4899', '#8B5CF6', '#06B6D4']
  );
...
function setKpiVariation(elId, current, previous, invertColors = false) {
  ...
  if (pct === 0) {
    el.textContent = '→ igual à sem. ant.';
    el.className = 'kpi-variation var-badge neutral';
    return;
  }

  const arrow = pct > 0 ? '↑' : '↓';
  const label = pct > 0 ? 'a mais' : 'a menos';
  const isGood = invertColors ? pct < 0 : pct > 0;
  el.textContent = `${arrow} ${Math.abs(pct)}% ${label} que sem. ant.`;
  el.className = `kpi-variation var-badge ${isGood ? 'up' : 'down'}`;
}
