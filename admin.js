...
document.addEventListener('DOMContentLoaded', () => {
  ...
  DS.onAuthChange(async user => {
    // Firebase dispara onAuthChange com null logo de início enquanto ainda
    // verifica o token. Aguardamos 800ms antes de redirecionar para garantir
    // que não é um falso negativo.
    if (!user) {
      if (authResolved) {
        // já confirmado: realmente não está logado
